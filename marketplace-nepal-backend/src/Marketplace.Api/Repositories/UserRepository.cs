...
namespace Marketplace.Api.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<MarketplaceUser?> GetByEmailAsync(string email)
        {
...
FROM dbo.users WHERE LOWER(email) = LOWER(@Email);";
        ...
        public async Task<MarketplaceUser?> GetByGoogleIdAsync(string googleId)
        {
    ...
    updated_at AS UpdatedAt,
    last_login_at AS LastLoginAt
FROM dbo.users WHERE google_id = @GoogleId;";
            return await _db.QueryFirstOrDefaultAsync<MarketplaceUser>(sql, new { GoogleId = googleId });
        }

        public async Task<MarketplaceUser?> GetByFacebookIdAsync(string facebookId)
        {
    ...
    updated_at AS UpdatedAt,
    last_login_at AS LastLoginAt
FROM dbo.users WHERE facebook_id = @FacebookId;";
            return await _db.QueryFirstOrDefaultAsync<MarketplaceUser>(sql, new { FacebookId = facebookId });
        }
