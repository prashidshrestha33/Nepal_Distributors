...
export class CategoryFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Dynamically add controls for each possible parent level
    for (let i = 0; i < this.maxVisibleLevels; i++) {
      this.form.addControl(`parent_level_${i}`, this.fb.control(null));
    }
  ...
  loadCategories(): void {
    ...
    this.companyService.getCategories().subscribe({
      next: (res: any) => {
        this.allCategories = Array.isArray(res) ? res : [];
        this.flatCategories = this.flattenCategories(this.allCategories);
