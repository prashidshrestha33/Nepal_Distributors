...
@Component({
  selector: 'app-homepage-static-values-catalog',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './homepage-static-values-catalog.component.html',
  styleUrl: './homepage-static-values-catalog.component.css'
})
export class HomepageStaticValuesCatalogComponent implements OnInit {
  ...
  load() {
    ...
    this.service.getStaticValuesCatagory().subscribe({
      next: (data: StaticValueCatalog[]) => {
        // Homepage catalog names — includes both 'Homepage' typed and 'Upload' typed ones
        const homepageCatalogs = [
          'aboutus', 'herobanners', 'services', 'contactinfo',
          'brand', 'advertisements', 'privacypolicy',
          'termsandconditions', 'testimonials', 'premierbrands'
        ];
        this.items = (data || []).filter(c =>
          c.catalogType?.toLowerCase() === 'homepage' ||
          c.catalogType?.toLowerCase() === 'hjomepage' ||
          c.catalogType?.toLowerCase() === 'upload' && homepageCatalogs.includes(c.catalogName?.toLowerCase() ?? '')
        );
      ...
      error: (err) => {
        console.error('Failed to load homepage static value catalogs:', err);
